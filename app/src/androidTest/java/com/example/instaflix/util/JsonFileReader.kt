...
fun mockResponse(code: Int): MockResponse =
    MockResponse().setBodyDelay(DELAY_RESPONSE, TimeUnit.SECONDS).setResponseCode(code)

fun mockResponse(fileName: String, responseCode: Int): MockResponse =
    MockResponse().setBodyDelay(DELAY_RESPONSE, TimeUnit.SECONDS).setResponseCode(responseCode)
